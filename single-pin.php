					...
					<div class="col-sm-12">
						...
						<div id="post-<?php the_ID(); ?>" <?php post_class('post-wrapper'); ?>>
									...
									<div class="post-actionbar">
										...
										<div class="ipin-share btn-group post-action-button">
											...
											<ul class="dropdown-menu">
	                                            <li><a href="" onclick="window.open('http://vkontakte.ru/share.php?url=<?php echo rawurlencode(get_permalink()); ?>', 'vk-share-dialog', 'width=626,height=500'); return false;"><i class="fa fa-vk fa-lg fa-fw text-info"></i> <?php _e('Share on VK.com', 'ipin'); ?></a></li>
	                                            <li><a href="" onclick="window.open('http://www.odnoklassniki.ru/dk?st.cmd=addShare&st._surl=<?php echo rawurlencode(get_permalink()); ?>', 'ok-share-dialog', 'width=626,height=500'); return false;"><div class="btn btn-primary btn-xs" style="padding: 0 1px; margin: 0 5px 0 3px;"><img width="12px" src="<?php echo get_template_directory_uri(); ?>/img/odnoklassniki.png" /></div> <?php _e('Share on OK.ru', 'ipin'); ?></a></li>
												...
												<li><a href="" onclick="window.open('https://twitter.com/share?url=<?php the_permalink(); ?>&amp;text=<?php echo rawurlencode(get_the_title()); ?>', 'twitter-share-dialog', 'width=626,height=500'); return false;"><i class="fa fa-twitter-square fa-lg fa-fw text-primary"></i> <?php _e('Share on Twitter', 'ipin'); ?></a></li>
												<li><a href="" onclick="window.open('http://www.reddit.com/submit?url=<?php echo rawurlencode(get_permalink()); ?>&amp;title=<?php echo rawurlencode(get_the_title()); ?>', 'reddit-share-dialog', 'width=880,height=500,scrollbars=1'); return false;"><i class="fa fa-reddit-square fa-lg fa-fw text-primary"></i> <?php _e('Share on Reddit', 'ipin'); ?></a></li>
							...
							<div id="post-featured-photo" class="post-featured-photo">
								...
								<div class="price_preorder">
								<?php if (of_get_option('price_currency') != '' && ipin_get_post_price() != '') { ?>
									<div class="post-content-price pull-right"><span class="badge badge-success"><?php echo ipin_get_post_price(); ?></span></div>
								<?php }	?>
								
								<?php
								//edited: send message
								$user_info = get_user_by('id', $post->post_author);
								$user_registeredemail = $user_info->user_email;
								$user_contactemail = get_user_meta($post->post_author, 'ipin_user_contactemail', true);
								
								if ($user_contactemail != '' && get_post_meta($post->ID, '_Preorder', true) != '') {	
								?>
									<div onclick="return false;" style="cursor:help;" class="post-preorder btn btn-default pull-right<?php if (current_user_can('subscriber')) echo 'hide'; ?>" rel="tooltip" title="<?php _e('You can contact Author to preorder', 'ipin'); ?>" data-placement="bottom"><i class="fa fa-cart-plus"></i></div>
								<?php } ?>
								</div>
					...
					?>
							...
							<div class="post-author-wrapper">
								...
								<div class="post-author-wrapper-header">
									...
									<?php
									//edited: send message
									if ($post->post_author != $user_ID && $user_contactemail != '' || ($post->post_author != $user_ID && current_user_can('administrator') && strpos($user_registeredemail, '@example.com') === false)) { ?>
									<button style="margin-right: 8px;" class="post-email btn btn-default pull-right follow<?php if (current_user_can('subscriber')) echo ' hide'; ?>" type="button"><?php _e('Send Message', 'ipin'); ?></button>
									<?php } ?>
									<a href="<?php echo home_url('/' . $wp_rewrite->author_base . '/') . get_the_author_meta('user_nicename'); ?>/">
								...
								<div class="modal-dialog">
									<div class="modal-content">
										...
										<div class="modal-body">
											...
											<div class="alert alert-success hide"><?php _e('Pin reported. Thank you for your submission.', 'ipin'); ?></div>
											<p></p>
							...
							<div class="modal ipin-modal" id="post-email-box" data-backdrop="false" data-keyboard="false" tabindex="-1" aria-hidden="true" role="dialog">
								<div class="modal-dialog">
									<div class="modal-content">
										<div class="modal-header">
											<button class="close popup-close" aria-hidden="true" type="button">&times;</button>
											<h4 class="modal-title"><?php _e('Send Message', 'ipin'); ?></h4>
											<div class="post-author-wrapper">
												<div class="pull-left">
													<a target="_blank" href="<?php echo home_url('/' . $wp_rewrite->author_base . '/') . get_the_author_meta('user_nicename'); ?>/">
													<?php echo get_avatar($post->post_author, '48'); ?>
													</a>
												</div>
												...
												<div class="post-author-wrapper-header">
													...
													<div class="user-profile-icons">
														...
														<a href="<?php echo esc_url($user_info->user_url); ?>" target="_blank"><?php echo parse_url($user_info->user_url, PHP_URL_HOST); ?></a> 
														<?php } ?>
													
														<?php if ($user_info->ipin_user_facebook) { ?>
														/ <a href="http://www.facebook.com/<?php echo esc_attr($user_info->ipin_user_facebook); ?>" target="_blank">fb</a> 
														<?php } ?>
									
														<?php if ($user_info->ipin_user_twitter) { ?>
														/ <a href="http://twitter.com/<?php echo esc_attr($user_info->ipin_user_twitter); ?>" target="_blank">tw</a> 
														<?php } ?>
									
														<?php if ($user_info->ipin_user_pinterest) { ?>
														/ <a href="http://pinterest.com/<?php echo esc_attr($user_info->ipin_user_pinterest); ?>" target="_blank">pinterest</a> 
														<?php } ?>
									
														<?php if ($user_info->ipin_user_googleplus) { ?>
														/ <a href="http://plus.google.com/<?php echo esc_attr($user_info->ipin_user_googleplus); ?>" target="_blank">g+</a> 
														<?php } ?>
														
														<?php if ($user_info->ipin_user_vkontakte) { ?>
														/ <a href="http://vk.com/<?php echo esc_attr($user_info->ipin_user_vkontakte); ?>" target="_blank">vk</a> 
														<?php } ?>
														
														<?php if ($user_info->ipin_user_odnoklassniki) { ?>
														/ <a href="http://ok.ru/<?php echo esc_attr($user_info->ipin_user_odnoklassniki); ?>" target="_blank">ok</a> 
														<?php } ?>
														
														<?php if ($user_info->ipin_user_contactskype) { ?>
														/ <a href="skype:<?php echo esc_attr($user_info->ipin_user_contactskype); ?>?call">skype</a>
														<?php } ?>
													</div>
												</div>
											</div>
										...
										<div class="modal-body">
											<?php if (current_user_can('administrator')) { ?>
												<?php if ($user_contactemail != '') { ?>
													<div class="radio">
														<label>
														<input type="radio" name="post-email-selection" id="post-email-selection" value="<?php echo $user_contactemail ?>" checked="checked">
														<?php echo $user_contactemail; ?> (Contact)
														</label>
													</div>
												<?php } ?>
												...
												<div class="radio">
													<label>
													<input type="radio" name="post-email-selection" id="post-email-selection" value="<?php echo $user_registeredemail ?>"<?php if ($user_contactemail == '') echo 'checked="checked"'; ?>>
													<?php echo $user_registeredemail; ?> (Registered)
													</label>
												</div>
											<?php } ?>
											
											<input class="form-control" type="email" id="recipient-email" placeholder="<?php _e('Your Email', 'ipin'); ?>" value="<?php echo get_user_meta($user_ID, 'ipin_user_contactemail', true); ?>" />
											<p></p>
											<input type="hidden" id="email-post-id" value="<?php echo $post->ID; ?>" />
											<p></p>
											<textarea class="form-control" placeholder="<?php _e('Your Message', 'ipin'); ?>"></textarea>
											<p></p>
											<div class="alert alert-warning hide"><?php _e('Please enter a valid email', 'ipin'); ?></div>
											<div class="alert alert-success hide"><?php _e('Message sent to', 'ipin'); echo ' ' . $user_info->display_name; ?></div>
											<p></p>
											<input class="btn btn-success btn-block btn-ipin-custom" type="submit" disabled="disabled" value="<?php _e('Send Message', 'ipin'); ?>" id="post-email-submit" name="post-email-submit">
											<input class="btn btn-success btn-block btn-ipin-custom hide" id="post-email-close"  type="submit" value="<?php _e('Close', 'ipin'); ?>">
