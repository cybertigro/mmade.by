...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['form_name'] == 'save_settings_form') {
	...
	if ($_POST['ipin_user_vkontakte'] != $user_info->ipin_user_vkontakte)
		update_user_meta($user_info->ID, 'ipin_user_vkontakte', sanitize_text_field($_POST['ipin_user_vkontakte']));
	if ($_POST['ipin_user_odnoklassniki'] != $user_info->ipin_user_odnoklassniki)
		update_user_meta($user_info->ID, 'ipin_user_odnoklassniki', sanitize_text_field($_POST['ipin_user_odnoklassniki']));
	if ($_POST['ipin_user_contactemail'] != $user_info->ipin_user_contactemail)
		update_user_meta($user_info->ID, 'ipin_user_contactemail', sanitize_text_field($_POST['ipin_user_contactemail']));
	if ($_POST['ipin_user_contactnumber'] != $user_info->ipin_user_contactnumber)
		update_user_meta($user_info->ID, 'ipin_user_contactnumber', sanitize_text_field($_POST['ipin_user_contactnumber']));
	if ($_POST['ipin_user_contactskype'] != $user_info->ipin_user_contactskype)
		update_user_meta($user_info->ID, 'ipin_user_contactskype', sanitize_text_field($_POST['ipin_user_contactskype']));
	if ($_POST['ipin_user_legaltype'] != $user_info->ipin_user_legaltype)
		update_user_meta($user_info->ID, 'ipin_user_legaltype', sanitize_text_field($_POST['ipin_user_legaltype']));
	if ($_POST['ipin_user_legalid'] != $user_info->ipin_user_legalid)
		update_user_meta($user_info->ID, 'ipin_user_legalid', sanitize_text_field($_POST['ipin_user_legalid']));
	if ($_POST['ipin_user_legaladdress'] != $user_info->ipin_user_legaladdress)
		update_user_meta($user_info->ID, 'ipin_user_legaladdress', sanitize_text_field($_POST['ipin_user_legaladdress']));
...
<div class="container">
	<div class="row">
		...
		<!-- Edited tips for mmade -->
				<div class="form-group">
					<label class="form-label" for="description"><?php _e('About', 'ipin'); ?></label>
					<textarea class="form-control" name="description" id="description" tabindex="50"  placeholder="<?php _e('about me', 'ipin'); ?>" ><?php echo $user_info->user_description; ?></textarea>
					<p class="help-block"><?php _e('Short description about you and your hobby', 'ipin'); ?></p>
				</div>
		<!-- End Edit tips for mmade -->
				<div class="form-group">
				...
				<div class="form-group">
					<label class="form-label" for="ipin_user_vkontakte"><?php _e('VK.com Username', 'ipin'); ?></label>
					<input class="form-control" type="text" name="ipin_user_vkontakte" id="ipin_user_vkontakte" value="<?php echo esc_attr($user_info->ipin_user_vkontakte); ?>" tabindex="100" placeholder="<?php _e('myvkontakte', 'ipin'); ?>" />
				</div>
				
				<div class="form-group">
					<label class="form-label" for="ipin_user_odnoklassniki"><?php _e('Ok.ru Username', 'ipin'); ?></label>
					<input class="form-control" type="text" name="ipin_user_odnoklassniki" id="ipin_user_odnoklassniki" value="<?php echo esc_attr($user_info->ipin_user_odnoklassniki); ?>" tabindex="100"  placeholder="<?php _e('myodnoklassniki', 'ipin'); ?>" />
				</div>
				
		<!-- Edited tips for mmade -->
				<div class="form-group">
					<label class="form-label" for="ipin_user_contactemail"><?php _e('Contact Email', 'ipin'); ?></label>
					<input class="form-control" type="text" name="ipin_user_contactemail" id="ipin_user_contactemail" value="<?php echo esc_attr($user_info->ipin_user_contactemail); ?>" tabindex="100"  placeholder="<?php _e('contact@example.com (invisible to others)', 'ipin'); ?>" />
					<p class="help-block"><?php _e('Invisible for others', 'ipin'); ?></p> 
				</div>
		<!-- End edit tips for mmade -->
				 
				<div class="form-group">
					<label class="form-label" for="ipin_user_contactnumber"><?php _e('Contact Phone Number', 'ipin'); ?></label>
					<input class="form-control" type="text" name="ipin_user_contactnumber" id="ipin_user_contactnumber" value="<?php echo esc_attr($user_info->ipin_user_contactnumber); ?>" tabindex="100"  placeholder="<?php _e('+375 29 7654321', 'ipin'); ?>" />
				</div>
				
				<div class="form-group">
					<label class="form-label" for="ipin_user_contactskype"><?php _e('Contact Skype', 'ipin'); ?></label>
					<input class="form-control" type="text" name="ipin_user_contactskype" id="ipin_user_contactskype" value="<?php echo esc_attr($user_info->ipin_user_contactskype); ?>" tabindex="100"  placeholder="<?php _e('myskypeid', 'ipin'); ?>" />
				</div>
				
		<!--Legal -->
		
				<h1><?php _e('Legal', 'ipin'); ?></h1>
					<p>
				<div class="form-group">
					<label class="form-label" for="ipin_user_legaltype"><?php _e('Individual/Artisan/Legal', 'ipin'); ?></label>
					<p class="help-block"><?php _e('Legal tips', 'ipin'); ?></p>
					
					<label class="radio-inline">
						<input type="radio" name="ipin_user_legaltype" value="individual"<?php if ($user_info->ipin_user_legaltype == 'individual') echo ' checked="checked"'; ?>> <span style="font-size: 1.1em">Физическое лицо</span>
					</label>
					<label class="radio-inline">
						<input type="radio" name="ipin_user_legaltype" value="artisan"<?php if ($user_info->ipin_user_legaltype == 'artisan') echo ' checked="checked"'; ?>> <span style="font-size: 1.1em">Ремесленник</span>
					</label>
					<label class="radio-inline">
						<input type="radio" name="ipin_user_legaltype" value="legal"<?php if ($user_info->ipin_user_legaltype != 'individual' && $user_info->ipin_user_legaltype != '' && $user_info->ipin_user_legaltype != 'artisan') echo ' checked="checked"'; ?>> <span style="font-size: 1.1em">Юридическое лицо</span>
					</label>
				</div>
				<div id="legalsection"<?php if ($user_info->ipin_user_legaltype == 'individual' || $user_info->ipin_user_legaltype == '' || $user_info->ipin_user_legaltype == 'artisan') echo ' style="visibility: hidden;"'; ?>>
					<div class="form-group">
						<label class="form-label" for="ipin_user_legalid"><?php _e('Legal ID', 'ipin'); ?></label>
						<input class="form-control" type="text" name="ipin_user_legalid" id="ipin_user_legalid" value="<?php echo esc_attr($user_info->ipin_user_legalid); ?>" tabindex="100" placeholder="*обязательно" />
					</div>
					
					<div class="form-group">
						<label class="form-label" for="ipin_user_legaladdress"><?php _e('Legal Address', 'ipin'); ?></label>
						<input class="form-control" type="text" name="ipin_user_legaladdress" id="ipin_user_legaladdress" value="<?php echo esc_attr($user_info->ipin_user_legaladdress); ?>" tabindex="100" placeholder="*обязательно" />
						<p>
					</div>
				</div>
				<h1><?php _e('Decor', 'ipin'); ?></h1>
					<p><p>
				...
				<div class="form-group">
					...
					<label class="onoffswitch-label" for="ipin_user_notify_follows" tabindex="150">
				...
				<div class="form-group">
					...
					<label class="onoffswitch-label" for="ipin_user_notify_likes" tabindex="120">
						<span class="onoffswitch-inner"></span>
						<span class="onoffswitch-switch"></span>
					</label>
					</div>
				...
				<div class="form-group">
					...
					<!-- Edited remove checkbox for mmade 
				...
				<div class="form-group">
					...
					<label class="onoffswitch-label" for="ipin_user_notify_comments" tabindex="130">
						<span class="onoffswitch-inner"></span>
						<span class="onoffswitch-switch"></span>
					</label>
					</div>  -->
...
jQuery(document).ready(function($) {
	$('#avatarform').css('top', $('#avatar-anchor').offset().top-110);
	...
	$('input[type=radio][name=ipin_user_legaltype]').change(function() {
		if (this.value == 'legal') {
			$('#legalsection').css('visibility', 'visible');
		} else {
			$('#legalsection').css('visibility', 'hidden');	
		}
	});
});
</script>
