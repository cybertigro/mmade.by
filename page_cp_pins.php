...
//edited: restrict price field for artisan & legal
$usertype = get_user_meta($user_ID, 'ipin_user_legaltype', true);
$legalid = get_user_meta($user_ID, 'ipin_user_legalid', true);
$legaladdress = get_user_meta($user_ID, 'ipin_user_legaladdress', true);
					...
					<form id="pin-edit-form">
						<?php if (of_get_option('form_title_desc') != 'separate') { ?>
							...
							if (of_get_option('htmltags') == 'enable') {
								echo ipin_wp_editor('pin-title', nl2br($pin_info->post_title));
							} else { ?>
						...
						<?php } else { ?>
							...
							<?php
							if (of_get_option('htmltags') == 'enable') {
								echo ipin_wp_editor('pin-content', nl2br($pin_info->post_content));
							} else { ?>
						...
						<?php if (of_get_option('price_currency') != '' && ($usertype == 'artisan' || ($usertype == 'legal' && $legalid != '' && $legaladdress != ''))) { //edited: restrict price field ?>
							...
							<div class="checkbox" style="margin: -6px 0 8px 60px;">
								<label>
								<input id="preorder" name="preorder" value="1" type="checkbox"<?php if (get_post_meta($pin_info->ID, '_Preorder', true) != '') echo ' checked="checked"'; ?>> <span style="font-size: 1.2em;"><?php _e('Needs to preorder', 'ipin'); ?></span>
								</label>
							</div>
						<?php } ?>
						...
						<?php echo ipin_dropdown_categories('', 'board-add-new-category'); //edited: remove Category for New Board  ?>
					...
					<div class="col-sm-8 usercp-wrapper usercp-pins">
						...
						<form id="pin-postdata-form">							
							...
							<?php if (of_get_option('price_currency') != '' && ($usertype == 'artisan' || ($usertype == 'legal' && $legalid != '' && $legaladdress != ''))) { //edited: restrict price field ?>
								...
								<div class="checkbox" style="margin: -6px 0 8px 60px;">
									<label>
									<input id="preorder" name="preorder" value="1" type="checkbox"> <span style="font-size: 1.2em;"><?php _e('Needs to preorder', 'ipin'); ?></span>
									</label>
								</div>
							<?php } ?>
							...
							<?php
							$parent_board_id = get_user_meta($user_ID, '_Board Parent ID', true);
							...
							<?php if ($boards_count != 0) { ?>
								<?php echo ipin_dropdown_boards(null, get_user_meta($user_ID, 'ipin_last_board', true)); ?>
							<?php } ?>
							
							<input<?php if ($boards_count == 0) echo ' style="display: block;"'; ?> class="form-control board-add-new" id="board-add-new" type="text" placeholder="<?php _e('Enter new board title', 'ipin'); ?>" />
							...
							<?php } ?>
							<?php echo ipin_dropdown_categories('', 'board-add-new-category'); //edited: remove Category for New Board ?>
							<a id="pin-postdata-add-new-board" class="btn btn-default btn-sm pull-right<?php if ($boards_count == 0) echo ' hide'; ?>"><?php _e('Add new board...', 'ipin'); ?></a>
							<input type="hidden" value="<?php echo $attach_id; ?>" name="attachment-id" id="attachment-id" />
							<div class="clearfix"></div>
							<input <?php if ($title == '') { echo ' disabled="disabled"'; } ?> class="btn btn-success btn-block btn-ipin-custom" type="submit" name="pinit" id="pinit" value="<?php _e('Pin It', 'ipin'); ?>" /> 
							<div class="ajax-loader-add-pin ajax-loader hide"></div>
						</form>
					</div>
					</div>
